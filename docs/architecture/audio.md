...
* [`audio_marker.py` — the capture-alignment marker](#audio_markerpy--the-capture-alignment-marker)
...
This is the host-side twin of `modes_irq.BANK_SWAP_CHUNK_SIZE`, which splits C64-side REC DMA for exactly the same reason and took NMI capture from 67 % to 97 %. The host path simply never got the same treatment.
...
The fix is the mirror of a check `_select_sid_entry` already made in the other direction: `auto` now resolves the live `$D400` owner via `active_socket_at_d400`, and a populated socket with no calibration for it gets `linear` rather than a table measured on different silicon. An UltiSID core owning `$D400` still gets the baked table — that pairing is correct.
...
`AudioStreamer.stats()` is the public snapshot of these counters (pushed/queued samples under `_count_lock`, plus the underrun/late-slot telemetry) — the seam tests and reporting read instead of the private attributes, whose single-owner write discipline stays visible at the attribute level. Two of its keys say something about scope: `late_worst_window_s` is named for the window `_maybe_log_health` clears it on, because every counter beside it is cumulative for the run; and `running` is the worker's liveness, which is what the crash handler's `running = False` is *for* — a caller can otherwise only infer a dead worker from silence.

#### One worker per start, and a join that says when it failed

`stop()` joins the worker with a bounded timeout (`WORKER_JOIN_TIMEOUT_S`, 1 s) because a ring write on a stalled link can exceed it — the caveats record this link's REST surface wedging for far longer. Two things follow from that, and neither was handled:

* **It has to say so.** The counters `stop()` clears immediately after the join are still being mutated by a surviving worker, and nothing else in the process reports the condition. A timed-out join now logs a warning.
* **The next start must not resurrect it.** The loop guard used to be the shared `running` flag, which the next scene's `start_*` sets back to `True` — so an orphan that was parked in a write when `stop()` gave up simply carried on, and two workers dripped into one ring with independent `write_addr` cursors, both feeding the same servo. `_start_worker` bumps a generation counter and hands it to the thread; the loop exits as soon as its own generation stops matching, which is a fence the next start cannot undo. Dropping `self._worker_thread` on a timed-out join is safe *because* of that fence.
...
**The staged upload is bounded by `REU_AUDIO_MAX_BYTES`.** One byte is one sample, so the region a track occupies grows with its duration and nothing about the upload bounds itself. Past ~20 minutes of source at the 12032 Hz NTSC default the payload runs into the video staging region (`modes_irq.REU_VIDEO_SCREEN_BASE`, `$E00000`) that the REU bank-swap bitmap path rewrites every frame — the pump would DMA bitmap bytes into the ring as full-scale garbage while the video writes shred the audio, with no host-side error, on an ordinary long clip. `_fit_reu_audio_region` truncates the payload (and with it the EOF pad, which starts where the payload ends) and warns. The ceiling is a local constant in `audio_handlers.py` rather than an import, so the audio layer keeps no dependency on the video layer; `tests/test_reu_audio.py` asserts it against `modes_irq`'s own base so the two cannot drift. The mic ring at `REU_MIC_BASE` and the sampler's PCM ring are *not* the binding neighbors — a scene runs at most one pump, so those regions are never live at the same time as a staged track.
...
Implemented across five modules (split 2026-08, one seam per job):
[c64cast/audio/dac_calibration.py](../../c64cast/audio/dac_calibration.py) owns the measurement run — bring-up, per-socket isolation, capture + retry — decomposed into module-level steps over a frozen `_RunContext` (`_bring_up_dac_env` → `_open_capture` → `_measure_one`/`_measure_each_socket` → always `_silence_and_reset` → `save_calibration` → `_report_run`), so the per-ring capture (`_capture_ring`) and per-SID measurement are plain functions a test can reach rather than closures.
[c64cast/audio/dac_slot_ring.py](../../c64cast/audio/dac_slot_ring.py) is the pure-numpy DSP: ring construction, extraction, every capture gate, and the sidtable fold — no hardware, no sounddevice, so the whole pipeline runs on synthetic captures in tests.
[c64cast/audio/dac_capture_device.py](../../c64cast/audio/dac_capture_device.py) picks and probes the sounddevice input.
[c64cast/audio/dac_calibration_store.py](../../c64cast/audio/dac_calibration_store.py) is persistence + identity keys.
[c64cast/audio/dac_curve_resolve.py](../../c64cast/audio/dac_curve_resolve.py) resolves `[audio].dac_curve` at playback.

Only the **emulated-UltiSID** table ships baked into [c64cast/audio/dac_curves.py](../../c64cast/audio/dac_curves.py). Hardware measurement (2026-07-02, Cam Link) showed the U64 FPGA UltiSID curve is deterministic across units, and that the 6581/8580 model knob is irrelevant — byte-identical output (corr 0.99999) — so one table generalizes. Its curve is all-positive, a valid digi shape with silence at a mid-level code.

The shipped bytes are the **re-bake of 2026-08-06** (Cam Link, `scripts/diags/mahoney_slot_ring_probe.py --source ultisid1`, 18 rings merged over two runs, `Digis Level = Medium` — the default). It reproduces the 2026-07-02 *curve* at corr 0.99957, confirming the earlier pass really did capture the emulated core, but not its *table*: the levels→code fold has been rewritten since, and the superseded bytes are non-monotonic through the very curve they came from (27 backward steps, worst −1.73% of span, against 0 for these). The raw signed-level curve and the table of record live in `scripts/diags/mahoney_measured_tables.json` (`ultisid.sidtable`).

Measuring the core at all needs it **isolated and unmuted** — see "Multi-socket U64/U2+" below for why address routing alone leaves the capture at the noise floor.

*Credits for the technique:* Pex 'Mahoney' Tufvesson (the technique and the white paper's §XIV environment block), Jürgen Wothke (the websid effective-bit analysis), Antonio Savona / Broken Bytes (the 48 kHz `$D418` article), and CodeBase64.
...
Every code is then measured against the **same baseline inside one capture**, so its signed level comes off the waveform directly and no sign has to be inferred. A stereo capture is folded to mono by averaging, so the SID pan setting only scales every measurement uniformly and cancels in the normalized ladder — no mixer changes are needed for it. `extract_slot_levels` does the rest and is pure, so it can be re-run offline against a saved capture:
...
**It would be wrong to read this as "link X can't calibrate."** The gate is a statement about the *data*: a rig reading in the healthy band is unaffected however it connects, and calibrating over a link with no config API is a supported, useful thing to do — the reported result of doing it is a considerable improvement. What such a link cannot do is switch a second chip out of the measurement, which is worth doing by hand in the machine's own settings first.

0.5% is 2.5× the healthy worst case. The failure carries its own type, `UnsteadyRingError`, because the advice inverts: every cause `capture_fault_message` lists is about the recording not carrying the ring, and here it demonstrably does — so `_unsteady_ring_message` leads with "the input is right" and asks instead what else is reaching that output.

##### Individual slots glitch; the ring does not

The fault behind every refusal seen in the field is a **single slot reading wrong on a single pass**. On all five captures kept for diagnosis — across both links — 1 to 6 codes out of ~86 read far off on exactly one pass while every other code agreed to **0.004%**. The ring itself replays; individual readings occasionally do not.

That matters because `pass_spread_frac` is a `max` over codes. One transient glitch out of ~260 readings pins it at 1–2%, so gating on it refused runs whose rings were otherwise perfect — a U64 run reading 1.93% had a median disagreement of 0.004% and exactly two bad slots. Two things follow, and both are needed:

* **The gate reads `pass_spread_p95_frac`,** the 95th percentile of the same per-code spread. A handful of glitched slots cannot move it, while a ring that genuinely is not replaying moves every code and therefore moves it too. Replayed against the kept captures, four of the five are accepted and the one with ten outlier codes is still refused. `pass_spread_frac` is still reported, as the worst-slot number it always was, and `pass_outlier_codes` counts how many cleared the healthy band.
* **`levels` is the median across passes, not the mean.** This is the half that matters for correctness. A mean folds the outlier into that code's level and the error survives into the ladder, which is exactly what a wrong entry sounds like. With three passes the median discards it outright. Gating alone would only have converted a bad table into a failed run.

**This was the original fault.** Before any gate existed, those glitched slots were averaged straight into the table, and the result is signal-correlated distortion — inaudible over a quiet passage, gross hiss once the material gets loud. Rejecting them is what fixes it; refusing the run never was.

The evidence is a same-chip cross-link comparison. Before: a TR+-measured table and an Ultimate-measured table of **the same 6581** agreed at corr 0.844 / 18% RMS of span, 12 of 256 entries identical. After: **corr 1.0000 / 0.12% RMS, 217 of 256 entries identical** — which is the figure two clean Ultimate runs of one chip reproduce at, twelve days apart. Two consecutive Ultimate runs now reproduce socket 2 at corr 1.0000 / 0.00% with 254 of 256 entries identical.

So the link was never the variable. A TR+ link measures the chip identically to the Ultimate once glitched slots are rejected, and the earlier readings that looked like "TR+ measures worse" were chance in which runs happened to catch a glitch.

##### Drift versus a ring that really is unsteady

Where the passes *do* disagree broadly, the reason still splits two ways with opposite fixes, so `_pass_gain_decomposition` fits one scalar per pass — the level the whole ring came back at on that lap — and reports what survives it (`pass_residual_frac`, measured the same robust way as the gate so the two compare directly). A residual well under the spread means the ring replayed faithfully and only the level it was measured *through* was moving: re-measure once the path has settled. A residual close to the spread means the laps genuinely differ. On synthetic captures the two land far apart — a 2–20% drift or a settling envelope reads 0.01–0.22 of the spread, random per-block gain jitter reads 0.70–0.89 — so `PASS_RESIDUAL_DRIFT_RATIO` splits them at 0.5.

##### What a whole run is worth

Rings that individually clear the gate still add up. A run whose rings sat in the marginal band produced a table measurably less reproducible than one with none: same chip, same hour, the run with one marginal ring reproduced at corr 0.9994 / 0.78% RMS where the clean run managed corr 1.0000 / 0.00%. `_marginal_run_summary` emits one line per run naming how many rings cleared `RING_SPREAD_HEALTHY` (0.2%) and the worst of them, and `rings_marginal` is persisted with the metrics. The table is still written — the count is a quality signal, not a verdict.

**Same-chip reproducibility is the yardstick, not table agreement.** Two clean measurements of one chip agree at corr 1.0000 / 0.12%; two *different* chips measured just as cleanly agree at corr 0.74 and −0.15, and their 256-entry `sidtable`s match on 6 entries. Any claim that a measurement is good has to be made against a repeat of that same chip.

**Over a link without the multi-SID config surface, nothing else gets muted for you.** `_isolate_socket` and `_isolate_mixer` both run only inside the `supports_sid_config` branch, so an Ultimate run measures with the target socket at unity and every other source — the second socket, both UltiSID cores, both sampler channels — forced to `OFF`, while a TR+ run on that same machine measures with all of them at whatever the user left. On a two-SID machine that means a live 6581 idling into the captured output. Worth muting by hand; it was *not* the cause of the refusals above, which were slot glitches.

#### Keeping the evidence

A refused capture is the only evidence for its own refusal, and reproducing it costs a ~50 s hardware run that may not fault the same way — twice a calibration was rejected on a number nobody could go back and explain, once because the file was deleted and once because the waveform was simply dropped on the way out of `_capture_ring`. `_save_unusable_capture` writes the refused waveform to `paths.unusable_capture_dir()` (`<data root>/calibration/unusable/<key>-<stamp>.npz`) with the codes, the sample rate and the diagnostics in the same file, since extraction needs all three to read it back; `np.load` plus one `extract_slot_levels` call reproduces the refusal offline. It is a diagnosis aid, so every failure in it is swallowed — it must never turn a measurement failure into a crash.

`_capture_ring` writes the ring once and re-records up to `RING_ATTEMPTS` (2) times, so a ring spoiled by a transient costs one capture window instead of the run; a rig that never produces a usable one then fails with `capture_fault_message`, which names the device it recorded from, how loud that recording was, the three things that cause it (wrong input / audio not routed / NMI never came up), and the input list to pick from. `cli` catches `MeasurementError` alongside `CaptureUnavailableError` for a clean **exit 3** — an unmeasurable rig is a setup problem, not a bug, and neither should traceback.
...
* **The Ultimate, live** — `active_socket_at_d400` reads the socket map, and its answer is authoritative *including* its `None`: nothing physical owns `$D400`, an UltiSID core does, and a physical-chip table must not be applied there.
...
At playback, `load_calibrated_table` picks the entry matching whichever socket is *currently* live-mapped to `$D400` — `active_socket_at_d400` does a live `SID Addressing` / `SID Sockets Configuration` read. That is what stops a calibrated physical-chip table from being misapplied when an UltiSID core actually owns `$D400`. With no live backend, or when the file has no socket-keyed entries, it falls back to the `"default"` entry, or to the lone entry if there is exactly one.

**A `"default"` entry from a link without the multi-SID config surface is an assumption, and it is stated.** `active_socket_at_d400` needs `profile.supports_sid_config`, which only the U64 has, so `run_calibration` on any other link (TeensyROM, or an Ultimate II+ with no SID addressing to read) cannot isolate a socket and cannot name the chip: it measures whatever answers `$D400` and files it under `"default"` with `detected: null`. On a single-SID machine that is exactly right. On a machine with a second chip, or with address mirroring on, the measured ladder is a blend of both — and a blended ladder is the same signal-correlated distortion as a mismatched one. Nothing on the host side can distinguish those two cases, so `load_calibrated_table` logs, once per resolution, which one it is assuming. Making it *knowable* needs a firmware-side SID identity query; the TeensyROM protocol has no such token today (its full token list covers launch, ping, SID playback control, file ops, reset, read/write C64 memory, and firmware check), so `ReadC64MemToken` plus a C64-side detect routine is the only avenue, and a native query is worth requesting upstream.

Resolution: `resolve_dac_curve_for_backend(cfg, be=...)` maps `"auto"` to the applicable calibrated table if present, else `mahoney_ultisid` on the Ultimate, else `linear`. It yields to an explicit `digi_boost` by staying linear. `"calibrated"` forces the table and raises if it is absent. The calibration key is resolved once per resolution and threaded through (`_resolve_auto_curve`), because deriving it can cost a live `GET /v1/info` on the Ultimate — it used to be recomputed for every log line that mentioned it.
...
Rather than assert a false "resolves to `mahoney_ultisid`", or a hard `calibrated`-missing error, it consults `dac_calibration_store.list_calibration_files(backend)` for any file on disk recorded for this backend and downgrades accordingly:
...
**The R-rate *observation* stays on regardless** (`RateServo.observe_r_rate`, split out of `update_rate_loop` for exactly this). Gating the measurement on the disabled controller left the one quantity that shows content-dependent tick loss reading zero in every log — a diagnostic hole, not a safety property; the guardrail is against *controlling* on R, not against looking at it. It costs nothing on top of the read the gap servo just did. The instantaneous per-chunk rate is kept alongside the EMA because the EMA says where the consumer sits while the spread between successive instantaneous values says how much it is *moving*, and a wandering R drags playback pitch with it through a gap servo that follows R faithfully by design.
...
**A discarded *pending* chunk leaves a ring span nothing else would write.** The one-chunk pipeline advances `write_addr` when it hands a chunk off, so when the next iteration finds the epoch bumped and drops that chunk, `[pending_addr, +chunk_size)` is already behind the next chunk's address: the NMI reads it one ring lap later as stale audio — an ~85 ms echo at the splice, in the one place the design promises a constant-latency crosscut. The worker therefore NEUTRAL-fills that span before continuing, which also restores `w_head`'s contract ("just past the last byte actually written") and keeps the servo from being handed a `W` a whole chunk behind the real head at the splice. The pause path's deferred stomp happened to cover this; a plain `flush()` (seek, loop wrap) did not.
...
* **`bands`** — Hann → `np.fft.rfft` → mean magnitude over log-spaced edges → `log1p` compression, clipped to [0, 1]. The band-edge function and the `log1p(mag * 100)` curve are **shared with the spectrum overlays** (moved here; both subclass `_spectrum._SpectrumBands`, which imports them), so the bars it draws and the bands the analyzer reports describe identical frequency ranges — one definition, not two that can drift.
...
**Teardown: independent guarantees, not a transaction.** All three sources in this module run their teardown through `run_teardown_steps` (`c64cast/_teardown.py`), for the reason [scenes.md](scenes.md#scenespy--scene-state-machine) gives at length: each step is a separate promise to the next scene, and `SourceScene`'s own teardown sits under `Playlist.safe_teardown`, so an unguarded raise here abandons every promise behind it *and* is silent. `MicAudioSource` joins its analyzer *in front of* the audio stop that keeps the next scene from inheriting a streaming pump; `AudioFileSource` joins its decoder *behind* that stop, because stopping the sink is what releases a decoder parked on a full queue. Neither join can raise any more — for different reasons, both worth knowing.

The file source's join *could*, and the fix is in `setup` rather than in the guard. `Thread.join` raises `RuntimeError` on a thread that was never started, and `setup` used to publish the decode thread to `self._thread` before calling `start()` — so a host out of threads left an unjoinable object exactly where teardown reaches for one. That was reached, not theoretical: `SourceScene.setup` catches an audio source that fails to start, logs, and flips `is_done`, so the playlist advances and tears the scene down through that join, and on the DAC path `start_for_external_source()` has already run by then — so with the join still in front of it, the starved step was the `audio.stop()`. `_start_decode_thread` now publishes only once the thread is running, which is the order `PollThread.start` uses and says why. The guard stays as defense in depth; the cause is gone. A decoder that survives the bounded join behind the audio stop stays referenced, and the next `setup()` *on that source* refuses to clear its stop event or start a second decoder until the survivor exits — a per-source fence, not a per-sink one: the `AudioStreamer` is shared across scenes where the sampler is built per scene, so the fence bounds what one source can stack on itself rather than what can reach the sink.

The mic source's never could. `AudioFeatureStream.stop` is a `PollThread.stop`, which returns normally on a join timeout (it keeps the reference and warns) and raises only the join-current-thread `RuntimeError` that `_pollthread` makes its lifecycle lock reentrant to produce — and nothing tears a mic source down from inside the analyzer's own tick. Its guard costs nothing and matches its two siblings, but it closed no live defect, and its test says so in its body. `SidFileAudioSource` had the sharpest version: one `try` wrapped the KERNAL-vector restore *and* the chip silences, so a REST hiccup on the vector left every SID ringing with only a log line to say so. Its step *order* is still load-bearing — the extra chips are zeroed before the SID-address config restore, because a U2+ emulated side moved home mid-note keeps ringing where no write can reach it, and a machine reset does not clear the emulation's voice state (HW-verified). Nothing in it suppresses the cursor blink: `suppress_cursor_blink()` was removed in #232 because poking BLNSW never held (the editor's input-wait loop overwrites that byte microseconds after the DMA lands), and the BASIC clear-and-loop PRG is what actually keeps the cursor off.

**Why a focused source, not `AVFileSource`.** `AVFileSource` hard-requires `container.streams.video[0]` (an audio file has none), and it carries the whole video/atempo/transport apparatus. `AudioFileSource` is a small decode loop — open, resample, push — plus the reactive-analyzer install lifted from `MicAudioSource`. It reuses `video.av_open`/`ensure_pyav` for the PyAV bring-up.
...
## `audio_marker.py` — the capture-alignment marker

Captured audio from a capture card is sample-accurate once rolling, but its time origin drifts against the source by an unknown offset — every capture session starts recording at a different moment than the C64 starts playing. Comparing captures against each other, or against the source, needs a shared anchor, so `[audio].source_alignment_marker` (a debug/capture-only flag) prepends a 100 ms marker to the REU pre-encode. The marker rides the front of the encoded 4-bit buffer, which is why only the REU-pump path carries it: the host-DMA path encodes chunk-by-chunk in real time and stays unmarked.

The marker needs two properties, and both rule out "just inject a beep". It must be *in-band*: prepended before upload so it plays through the exact same NMI → `$D418` → capture pipeline as the content — marker artifacts then mirror real-audio artifacts, which an anchor spliced host-side into the capture could never do. And it must be *unmistakable*: a 100 ms linear chirp, 200 → 3500 Hz (`MARKER_DURATION_S`, `MARKER_FREQ_START_HZ`/`_END_HZ`), has a sharp autocorrelation peak and doesn't occur in natural material — nothing musical or spoken sweeps that range in 100 ms. The duration is the trade: long enough for clean correlation SNR under capture noise, short enough that the pre-roll blip barely registers; the band fits under the ~6 kHz Nyquist of the 12 kHz default sample rate with margin against aliasing.

Finding it is `find_marker_in_capture`: FFT cross-correlation of the capture against `synthesize_capture_reference`, which models what the capture device actually sees — not the ideal chirp but a *staircase*, each 4-bit code held for `capture_rate / playback_rate` output samples, because the 6510 only writes `$D418` at the NMI rate (sample-and-hold). Both signals are mean-subtracted first, so unknown input gain and DC offset drop out of the peak position; source-time-zero is the peak plus `MARKER_DURATION_S`. The rates are parameters rather than constants because the module predates the current default (`DEFAULT_PLAYBACK_RATE` is still the old 8000): the injection site passes the streamer's [**effective** rate](#sample_rate-is-a-request-effective_rate-is-what-you-get) — the same achieved-not-nominal rate the whole pre-encode uses — so the marker and the content stay on one clock.
