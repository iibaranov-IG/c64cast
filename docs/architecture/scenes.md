...
**Teardown steps are independent guarantees, not a transaction.** A scene's teardown owes the next scene a clean machine, and every promise in it — the display restored, the audio pump stopped, a capture handle released, a cartridge reset — stands on its own. Sequencing them in one `try` (or one bare statement list) means the first raise silently abandons the rest, and `Playlist.safe_teardown` swallows that raise, so the next scene starts on dirty hardware with nothing in the log to say why. `run_teardown_steps(log, who, steps)` — in `c64cast/_teardown.py`, at the package root beside `_pollthread`, because the audio sources owe the same promises one layer down — takes `(label, callable)` pairs, guards each one, and logs `teardown step '<label>' failed; continuing` at ERROR. It catches `Exception` and not `BaseException` deliberately: teardown runs on the shutdown path, and a `KeyboardInterrupt` must propagate rather than be logged as a failed step and dropped. Every teardown in this module that owes more than one guarantee runs through it — `WebcamScene`, `SourceScene`, `VideoScene`, `LauncherScene` here, and the three SID scenes in [sid.md](sid.md#waveformpy--sidemupy--sid_host_emupy--sid-oscilloscope-scene), whose section also covers what their step *order* buys. So do the three audio sources a `SourceScene` composes — see [audio.md](audio.md#audio_sourcepy--audiofilesource-audio-file-reactive-source). Where a handle has to be dropped, the field is cleared before the call, so a `stop()` that raises does not leave a dead worker referenced. That holds where the teardown is the only reader of the field, and **not** where something else reads the handle's *liveness*: `PollThread.stop` deliberately keeps its thread reference when a bounded join times out, because a cleared one makes `is_running()` report False while the worker runs, and the next `start()` trusts that and clears the stop event the survivor is still reading. `AudioFileSource` was the one site here on the wrong side of that line, and #388 moved it across: teardown runs the `audio stop` *in front of* the decode-thread join, `_join_decode_thread` keeps the reference when the bounded 2 s join times out, and `setup` raises on a live predecessor rather than clearing the stop event under it. That order is load-bearing: `UltimateAudioSampler.push_samples` retries until the *sampler* stops rather than on a bound of its own, so this source's `_stop` cannot release a decode thread parked on a full queue — the audio stop is what frees it, and behind the join it spent the full 2 s first. (The 200 ms bound that would make the join look ample either way is `audio_handlers.QUEUE_PUT_TIMEOUT_S`, the 4-bit DAC path only.) Nor does a down link strand the thread: `UltimateAudioSampler.stop` sets its stopped flag before any link I/O, so the `audio stop` step frees it whether or not its `gate_off` raises. What the fence is for is the survivor no step order can release — `_decode_loop` tests `_stop` before every push, but `resolve_file_spec` passes an `http(s)` media URL straight through to `av_open`, so a server that stops responding blocks inside `container.demux` where no check can see it, on the DAC path too.

Two boundaries on that rule, both deliberate. **A scene that owes one guarantee needs no runner**: `Scene.teardown` guards its own display-mode call, so `BlankScene`, whose only other statement is a trailing `audio.stop()`, has nothing that can starve anything — converting it would buy an untestable claim. `SlideshowScene` defines no `teardown` at all and inherits the base one unchanged. And **the runner catches `Exception`, not `BaseException`**, which costs one case the `try/finally` in `SourceScene` used to cover: a `KeyboardInterrupt` inside a step now propagates instead of running the steps behind it. That is the right trade — teardown is the shutdown path, and an interrupt logged as a failed step and dropped would hang it — and it is narrow in practice, because playlists run on a worker thread, where `KeyboardInterrupt` is not delivered.

**Guarding a step frees its position, but not always.** Because a failing step no longer starves the ones behind it, step order stops being a correctness property and becomes a readability one — with the exceptions noted here, each of which a tidy-up reorder would break silently. `AsidScene` closes its MIDI port before the restores, for the reason its [sid.md section](sid.md#waveformpy--sidemupy--sid_host_emupy--sid-oscilloscope-scene) gives. And **`VideoScene` logs its A/V lag summary before the audio stop**: `AudioStreamer.stop` clears its pushed-sample count and `UltimateAudioSampler.position_seconds` short-circuits once stopped, so a summary read afterwards divides a zero and reports `clock/wall=0.0000`. That gauge is the only clock/wall figure at `-v` — the live one is DEBUG — and `scripts/diags/mhires_tempo_clock_ab.py` parses it as `clock_final` when calibrating `[audio].dac_bitmap_tempo_hires/mhires`, so zeroing it would break a documented instrument rather than a log line. And **`AudioFileSource` stops its audio sink in front of its decode-thread join**, for the reason the paragraph above gives. Each has its own guard: reversing the port close reddens `test_the_midi_port_closes_before_the_machine_is_restored` in `tests/test_asid_scene.py`, moving the summary behind the audio stop reddens `test_the_av_lag_summary_reads_the_clock_before_the_audio_stops`, and putting the decode-thread join back in front of the audio stop reddens `test_file_audio_stops_the_sink_before_joining_decode` — the last two both in `tests/test_teardown_steps.py`. No test covers another's subject.

Overlays are a separate path and already safe: `Playlist.safe_teardown` wraps **each** overlay's `teardown` in its own `try`, so one failing overlay cannot starve another or the scene teardown after them.
...
**`transport_loop_info()`/`transport_loop_slots()`** (Live DJ/VJ Phase 7) are read-only additions for the web console's `TransportBar`, not new engine state: `loop_info()` returns `{state, a, b}` straight off the same `_loop_a`/`_loop_b`/`_loop_state` fields the record/stop machine above already owns, in content-seconds (`clock_to_content`) like `position()`; `loop_slots()` lists which pad numbers `self._loop_store` holds, mirroring `PerformanceSession.saved_look_slots()` so a console lights a recall pad only where there is something to recall. `perf_console._transport_dict` folds both into the state frame alongside `position`/`duration`/`frozen` (`transport_is_paused()`), and is `None` when the current scene declares none of the four required methods — a generator or a picture has no transport bar to draw.

### Scene fade timeline (Playlist)

When `[playlist].fade_duration_s > 0`, the Playlist's [`SceneFades` collaborator](config.md#playlist_supportpy--playlist-collaborators) drives the per-mode fade described in the `modes/` note. **Fade-in** overlaps the opening live frames: `begin_fade_in` (in `Playlist.safe_setup`) sets the mode's `fade_alpha = 0`, and `advance_fade_in` (top of `run_one_frame`) ramps it 0→1 over `fade_frames(scene)` frames (`round(fade_duration_s / frame_time)`), so the scene plays from frame 0, rising out of black. **Fade-out** is freeze+dim: on a *normal* scene end (not skip), `fade_out` runs just before `safe_teardown` in `_advance`, re-pushing the cached last frame (`repush_faded`) at descending alpha for the same span. **CTRL skip aborts both immediately** — the skip branch in `run_one_frame` calls `cancel_fade_in` (snap to full brightness) and sets `ended_via_skip` (suppresses the fade-out); a skip *during* a fade-out breaks the loop and consumes the event so it doesn't also skip the next scene. The fade applies to every compose-based scene including the interstitial (each rises from and sinks to black); non-compose scenes and the reload/broadcast teardown paths are unaffected.
...
* **Bitmap (hires/mhires) frame-pushing scenes** — `video`, live `webcam`, and `generative` at *every* `audio_source` — default to **20 fps** (both NTSC and PAL) while streaming the 4-bit `$D418` digitized-audio DAC, and to **half** the system rate (30 NTSC / 25 PAL) when muted. A bitmap mode re-uploads a full ≈9-10 KB frame every frame; when the audio worker is also writing the ring, the two write streams compete for the bus and the picture tears at full rate. **Exception — video on the off-bus Ultimate Audio sampler:** that audio doesn't touch the bus and forces the bus-clean REU-staged video path, so neither cap applies — it uncaps to the full system rate (60/50), which dedup turns into source-rate playback (see the `sampler.py` "fps" note). Char modes (petscii/mcm/blank) push a cheap delta-cached 1 KB screen, so they keep the system default — **unless the source repaints every tick and the DAC is streaming** (next bullet). Resolved by `config._frame_push_default_fps`; revisit the DAC/muted caps once firmware stops halting the CPU on DMA writes.
  * **The muted cap does not depend on `audio_source`,** and for a while two branches read it as if it did. `audio_source = "none"` (in `_build_generative_live`) and `"listen"` (in `_build_generative_listen`) skipped the cap entirely, so they were the only always-fresh bitmap scenes in the tree pushing ~9-10 KB at the full system rate — `"none"` with a test pinning it that way, `"listen"` with nothing pinning it at all. Both rationales named the DAC ("no DAC → not in scope", "no DAC stream to frame-cap against"), but no other builder uses that criterion: `_build_wled` has no audio whatsoever and caps, `_build_generative_sid` caps, and a `mic` scene built with the global streamer off caps. Host-DMA tear is caused by the frame push, and a silent generator pushes exactly as much of it as a loud one. Both now cap.
  * **`audio_src`, not the streamer handle, decides whether digitized audio is on the bus.** `_resolve_live_audio` returns the shared `AudioStreamer` whenever `[audio]` is on — including for `audio_source = "none"`, which pushes nothing to it — so testing the handle for `None` would hand a silent scene the 20 fps DAC cap. `_build_generative_live` asks the resolved `AudioSource` instead (`not isinstance(audio_src, NullAudioSource) and not file_uses_sampler`), which is the question the cap actually turns on.
* **Always-fresh scenes on the DAC take the 20 fps cap in char modes too** (`_frame_push_default_fps(..., always_fresh=True)`). "Cheap char mode" assumes a source with something to dedup, and `generative` and live `webcam` have none: the generator runs per tick and every camera grab differs, so the full screen (plus color RAM, on `mcm`) is rewritten every frame. At the system rate that traffic shares the one DMA socket with the audio ring writes and jitters the NMI service — HW 2026-07-25, an `mcm` generative scene with file audio on the DAC is a noisy mess at 60 fps and clean at 20. `VideoScene` is unaffected (it re-pushes only on a genuinely new source frame), as is off-bus sampler audio, which doesn't contend at all; both keep the high default.
...
## Composable scenes — `scenes.SourceScene` + `frame_source.py` + `generators/` + `effects.py` + `audio_source.py` + `modulation.py` + `music_features.py`
...
#### `generators/` — the `GenerativeSource` registry
...
**Package layout.** One module per source; `__init__.py` holds the base class + registry and imports every submodule **eagerly, in the historical declaration order** — unlike `overlays/`' lazy `_load_all()`, because (a) `generator_names()` promises that order (`test_introspect` pins tuple equality against `config._GENERATIVE_SOURCE_CHOICES`), so the import block is `# isort: off`-guarded and must not be re-sorted, and (b) the classes are public package attributes (`generators.PlasmaSource`) with only hard deps (numpy/cv2), so laziness would buy nothing and break `REGISTRY` readers like `introspect`. `_noise.py` carries `periodic_value_noise`, the tileable turbulence shared by `fire` and `soap`. Adding a generator = new submodule + one line at the **end** of the import block (appending keeps every existing choice's position stable).
...
Escape-time iteration count is fixed regardless of zoom depth, deliberately not grown toward the precision limit: the output quantizes to a 16-color C64 grid, so filament-level deep-zoom detail would be invisible anyway, and fixing it bounds per-frame cost at any depth. Interior (non-escaping) points are forced to black per-pixel, overriding the reactive brightness there.
...
* **`hopalong`** — iterates Barry Martin's chaotic point-map (`x'=y-sign(x)*sqrt(|b*x-c|)`, `y'=a-x`) for thousands of starting points in parallel, numpy-vectorized across the batch (the map itself is sequential in the iteration count), into a log-scaled density accumulator. It re-runs the whole batch from scratch every frame — cheap, a few hundred vector ops — so a continuous drift of the `a`/`b` constants reshapes the attractor immediately. The live knob over the base `a` is exposed as `source.shape`, named for the sweep's visible effect rather than the map's letter. Chaotic sensitivity means even a small constant change visibly reshapes the whole point cloud. Driven by a slow sinusoid at rest, `level`/`beat_phase` continuously, and `onset` as a one-frame kick.
...
The first batch of a WLED-effect-port initiative, from a survey of WLED's `wled00/FX.cpp` 2D matrix effects cataloged for porting.
...
The batch the port catalog flagged as needing an architecture decision, since generation N of a real simulation cannot be computed without generation N−1 — unlike every generator above.
...
**The `modulation is None` path is byte-stable**: `trails` uses its configured decay; `pulse`/`rgb_shift`/`strobe` are the identity transform; `invert`/`mirror`/`posterize` are non-reactive and deterministic regardless. Non-reactive scenes, the offline renderer, and the determinism tests are therefore unchanged. `pulse`/`rgb_shift`/`strobe` only visibly react when handed a live feature stream — a `generative` scene with `audio_source = "sid"`, `"mic"`, or `"file"` (`mod_source = "audio"`), or *any* scene under `mod_source = "clock"` once a `[performance]` beat grid is running (which, with `tempo_source = "audio"`, the live-input beat itself drives).
...
A scene holds an **ordered chain** `scene.effects: list[FrameEffect]`, not a single effect. `_render_with_overlays` runs the chain in order via `_apply_effect_chain`, at the single seam in the render path. `scene.effect` is a **compatibility property** over `effects[0]` (get) / a one-element chain (set), so the WLED bridge and any single-effect caller need no chain awareness. Config authors pick **either** the single `effect` **or** the `effects` list (mutually exclusive, validated in `validate_scene_cfg`); both bottom out in the same chain in `build_scene`.
...
* **`mod_source`** — which `MusicModulation` feeder drives this reactive layer: `"audio"` (the scene's own feature stream — SID host-emulator or live-input analyzer — as `modulation`), `"clock"` (the Phase-1 `TempoClock` via `scene.clock_modulation`, stamped on by `Playlist.safe_setup`), or `"off"` (always the `None` baseline). `_effect_modulation` resolves it per layer; a non-reactive effect ignores the arg regardless. Set once per scene by `build_scene` (from `[[scenes]].mod_source`) onto every layer — the "synced to audio OR quantized via MIDI tempo" requirement, with no new effect code.

A layer that raises in `apply()` is **dropped from the chain** — not the whole scene torn down, and not the entire effect surface nulled — so one bad effect can't kill a live set; the loop iterates a copy so the removal is safe mid-iteration.
...
* `audio_source.py` — `AudioSource` protocol (`setup`/`teardown`/`position_seconds`/`features`). `NullAudioSource` (silence), `MicAudioSource` (live audio input — reactive by default, see [`audio_features.py`](audio.md#audio_featurespy--audio-input-music-features-reactive-visuals-from-live-input)), `AudioFileSource` (decodes an audio **file** via PyAV to the 4-bit DAC AND runs the same pre-DSP analyzer over it, so a generative visual reacts to the track — the audio half of `c64cast tune.mp3`, `audio_source = "file"`; see the [`audio_source.py` file-source note in audio.md](audio.md#audio_sourcepy--audiofilesource-audio-file-reactive-source)), `SidFileAudioSource` (plays a `.sid` on the real chip — the audio half of WaveformScene, factored out so it pairs with any FrameSource). A SID-audio SourceScene is forced to host-DMA (`force_host_dma`) and fixed at VIC bank 0, so the payload must clear the display ($0400 for char, +$2000 for bitmap) — **char displays (mcm/petscii) are the most robust pairing**, but a bitmap display (mhires/hires) works with a tune that loads high enough to clear $2000. `run_sid_player` kicks its player via the firmware's `run_prg`, which re-inits the machine back to **text mode** — and `Scene.setup` configures the display's VIC registers *before* `audio_source.setup()` runs the player. So `SidFileAudioSource.resets_display = True`, and `SourceScene.setup` re-asserts the display mode *after* the audio source starts (mirroring WaveformScene's "VIC setup after the player" order); without this a bitmap mode renders its `$0400` color nibbles as PETSCII. SID-structural helpers live in `sid_host_emu.py` (re-exported by `waveform.py`). `SidFileAudioSource.setup` also runs SID Player Autoconfig (`sid_autoconfig.apply_sid_autoconfig`) before `run_sid_player`, matching the tune's requested chip model to the U64's actual SID hardware — see the "SID Player Autoconfig" bullet in the `waveform.py` section below for the mechanism.
...
It reads **`scene.features()` first**, not the `AudioStreamer`. That inversion is the whole point: FFT-ing the streamer inside the overlay makes it `REQUIRES_AUDIO`, hence blank on a SID/waveform scene — the chip makes the sound, there is no streamer — while *duplicating* analysis on a mic/file scene, where [`audio_features.AudioFeatureStream`](audio.md#audio_featurespy--audio-input-music-features-reactive-visuals-from-live-input) has already run the identical FFT one layer down. `features()` is the source-agnostic seam every reactive scene already exposes, so the overlays read that and the FFT becomes a fallback.
...
2. **Voice synthesis** — the SID producers (`music_features.SidFeatureStream`, `WaveformScene.features`) read envelopes and oscillator frequencies, not a spectrum, so their `bands` is empty **by design** (see modulation.py). Rather than leave a tune blank, `voice_bands()` places each *gated* voice's real frequency into a log-spaced Hz band over 40 Hz–8 kHz, deposits the snapshot's `level` there (max-combined, with a modest skirt into the neighbors so three oscillators across eight bands don't read as disconnected blips), and calls it a spectrum. It is a three-oscillator approximation of one — which for a chiptune is most of what the spectrum actually is. What the eye reads is the *frequency* motion: the bass holding low while the lead walks up the bands.
   This lives in the overlay layer deliberately. Filling `MusicModulation.bands` in the SID producers instead would make `bass`/`mid`/`treble` non-zero on the SID path and change every SID-reactive generator (`generators/` feeds both into hue and value) — which is exactly what the empty-bands invariant documented in modulation.py exists to protect. The display layer wanting a spectrum is not a reason to change what the feature struct claims to know.
3. **The direct FFT** — `features()` is None but an `AudioStreamer` is attached (a `reactive = false` mic scene, a webcam scene with audio on). Note this is the *post*-DSP tap on purpose: with no upstream analyzer to defer to, it's a scope on what the C64 is actually playing, where the feature analyzer needs the pre-DSP signal.
4. **Zeros** — no source at all. The overlay paints nothing and logs one warning naming the scene, since a silent no-op would otherwise look like a rendering bug.
...
* **`SetupProgressBar` + `bar_style_for`** — the painter and the per-mode fill. petscii/blank draw a row of `0x4E` "/" glyphs colored white; MCM exploits its synthesized charset (screen code = 2×2 crumb pattern, so `$C3` fills the top-left + bottom-right quadrants) with color-RAM byte `$09` (multicolor bit + white); hires/mhires get true 45° stripes — 8-byte cells cycling `$88 $11 $22 $44` set exactly the bits where `(x + y) % 4 == 0`, continuous across cells because a cell is 8 pixels wide — with the screen-nibble byte matched per mode (`$10` hires, `$11` mhires so both bit-pair sources render white). An unrecognized or absent mode yields no bar.

* **`SegmentedProgress`** — folds per-step progress into one fraction. Only two setup steps have real denominators (pre-scan samples, REU upload bytes); the rest jump to done via `complete()`. Each step carries a static weight, the overall fraction is the weighted average, and per-segment values are clamped monotonic so a noisy reporter can never walk the bar backward. `SegmentedProgress.off()` is the null object (`reporter()` → None, mutators no-op), so `VideoScene.setup()` stays unconditional whether or not a bar is drawn. `finish()` forces the right edge before playback starts.

The painter deliberately writes via `write_memory_file`, **outside** the `write_region` delta cache, and never erases itself. That works because every display mode's `setup()` calls `invalidate_cache()` and clears its field with uncached bulk writes — so the mode's first real frame push finds an empty cache, pushes the full region, and wipes the bar wherever it lives (char screen, bank-0 bitmap, or a staged bank about to be swapped away). Painting through region IDs instead would seed caches that the mode's own pushes don't know about; painting *around* the cache means there is no entry to go stale. Repaints are cell-quantized and monotonic — each writes only the newly filled span, ≤ 40 spans over a whole setup, noise against the ≈200 writes/sec DMA budget. The scene fade is also a non-issue: `fades.begin_fade_in` runs after `setup()` returns and the bar is never in the mode's `last_buffers`, so it neither fades nor replays.
